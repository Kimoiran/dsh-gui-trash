...
/**
 * 判断"该用哪个 dsh"：显式 launchCommand 优先；否则从 dsh 自身入口
 * （process.argv[1]）向上找 checkout。
 */
function resolveLaunch(config) {
  if (config.launchCommand) return config
  ...
  return { ...config, launchMode: 'installed', launchRoot: undefined, launchCommand: 'dsh web' }
}
